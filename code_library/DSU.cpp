...
struct BipartiteDSU {
    ...
    int makeUnion(int a, int b) {                                   // Union making with compression
        ...
        if(u_list[x] > u_list[y]) {                                 // Makes x root (y -> x)
            u_set[y] = x;
            u_list[x] += u_list[y];                                 // Root's size is increased
            u_color[y] = (u_color[a] + u_color[b] + 1)&1;           // Component's weight is changed according to root
            return x;
        }
        else {                                                      // Makes y root (x -> y)
            u_set[x] = y;
            u_list[y] += u_list[x];                                 // Root's size is increased
            u_color[x] = (u_color[a] + u_color[b] + 1)&1;           // Component's weight is changed according to root
            return y;
    }}
